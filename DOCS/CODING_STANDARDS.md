# Coding Standards: Playlist Downloader 3.0 (Qt 6 / C++20)
...
| Compiler | GCC ≥ 12 / Clang ≥ 15 / MSVC 2022. Warnings on: `-Wall -Wextra -Wpedantic -Wshadow -Wconversion -Wnon-virtual-dtor -Wold-style-cast`. Warnings are errors in CI (`-Werror` behind `PLDL_WERROR` option). |
...
│   │   ├── theme/
│   │   ├── downloads/          # yt-dlp engine provisioning, job model, option → argv mapping
│   │   ├── blocking/           # ad/tracker host lists, SponsorBlock client
│   │   └── ...
│   ├── web/                    # WebEngine layer: profile, page, view, scripts, interceptor
│   │   └── scripts/            # *.js injected into YouTube (resource files, never string literals)
...
Every directory is its own `add_library(pldl_<name> STATIC)` target with explicit
...
|---|---|---|
| Namespace | `pldl::<layer>` | `pldl::core`, `pldl::ui` |
...
* **Logging**: `Q_LOGGING_CATEGORY` per module (`pldl.web`, `pldl.downloads`, …). `qDebug()`
...
YouTube changes without notice. The rewrite treats every injected script as a
...
1. Scripts live in `src/web/scripts/*.js`, are loaded with `qt_add_resources`, and injected via
   ...
   **profile-level** collection (`profile->scripts()`), registered before the first load.
   ...
   pass data through `QWebChannel` or through a single `window.__pldl` config object
...
1. Scripts may rely only on the *stable tier*: standard web APIs, our own `<style>` ids,
   YouTube's **data contracts** (InnerTube JSON field names such as `adPlacements`,
   `streamingData`, `videoDetails`, the `yt-navigate-finish` event, `ytInitialPlayerResponse`,
   `ytcfg`, the `<video>` element and the documented player API on `#movie_player`), and the
   semantic custom-element tags YouTube ships (`ytd-*`, `ytp-*` classes, `ytlr-*`). Obfuscated
   class names, localised text and React/Polymer internals are rejected in review.
...
8. `QWebEnginePage::javaScriptConsoleMessage` is routed to the `pldl.web.js` logging category.
...
* No migration of 2.x settings except the account id and the download folder (ADR-002); a fresh profile is acceptable.
...
| Hardcoded user-agent strings spread around | breaks with YouTube changes | one constant, one override |
