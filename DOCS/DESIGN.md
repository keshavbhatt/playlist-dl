...
(Ctrl+,), Account and Help (a menu: Keyboard shortcuts, Online guide, Report a bug, About; owner requests 2026-09-24 and 2026-09-25). The rail hides while a video is full screen. Every action is also in
...
- No source chip: the engine is the only search source (ADR-003 revised); the engine setup
  sheet covers the first search on a fresh install.
