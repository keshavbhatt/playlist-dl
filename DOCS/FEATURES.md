...
| B4 | Paste a playlist or video link in the field: a YouTube playlist resolves to the Playlist page, a YouTube video opens the Download options sheet; a link on any other site is probed and the engine's answer decides (ADR-006) | "Process Playlist" button | KEEP | done (`SearchPage::linkOf`, `MainWindow::openVideoOptions`) |
