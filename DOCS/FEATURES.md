...
|---|---|---|---|---|
| S1 | One window: rail with Search, Playlist, Browser, Downloads; Settings, Account and a Help menu at the bottom; the rail expands to show labels (toggle under the logo, Ctrl+B, animated, remembered) (DESIGN.md) | toolbar + sliding pages | KEEP | done (`ui::MainWindow`, `ui::SideRail`, `ui::Actions`) |
...
| B4 | Paste a playlist or video link in the field (also by drag and drop onto the window, and offered from the clipboard on activation): a YouTube playlist resolves to the Playlist page, a YouTube video opens the Download options sheet; a link on any other site is probed and the engine's answer decides (ADR-006) | "Process Playlist" button | KEEP | done (`SearchPage::linkOf`, `MainWindow::openVideoOptions`) |
...
| E4 | A playlist is one job with entries; per-entry progress and the aggregate on the card; failed entries retried alone; a failed card shows its reason, Retry failed in the header, the items sheet queues the missing items; right-click menus mirror the hover buttons | one process per item, counters in QSettings | KEEP | done (one job per playlist, `DownloadJob::detailLine` "3 of 12") |
