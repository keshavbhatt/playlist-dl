...
#   PLDL_RT              runtime content snap root (kf6-core24)
#   PLDL_RT_LIB_PATH     LD_LIBRARY_PATH value for running dev binaries
#   pldl_prepare_runtime_farm <build-dir>
#   pldl_export_runtime_env  <build-dir>
#
...
PLDL_RT=/snap/kf6-core24/current
PLDL_CORE24_BASE=/snap/core24/current/usr/lib/x86_64-linux-gnu
...
pldl_prepare_runtime_farm() {
    local build="$1"
    local farm="$build/core24-libs"
    mkdir -p "$farm"
    local f b
    for f in "$PLDL_CORE24_BASE"/*.so*; do
        b="$(basename "$f")"
        case "$b" in
...
pldl_runtime_lib_path() {
    ...
    local rt="$PLDL_RT/usr/lib/x86_64-linux-gnu"
    echo "$rt:$rt/libproxy:$rt/pulseaudio:$build/core24-libs"
}

pldl_export_runtime_env() {
    local build="$1"
    export LD_LIBRARY_PATH="$(pldl_runtime_lib_path "$build")${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
    export QT_PLUGIN_PATH="$PLDL_RT/usr/lib/x86_64-linux-gnu/qt6/plugins"
    export QTWEBENGINEPROCESS_PATH="$PLDL_RT/usr/lib/qt6/libexec/QtWebEngineProcess"
    export QTWEBENGINE_RESOURCES_PATH="$PLDL_RT/usr/share/qt6/resources"
    ...
    export SPA_PLUGIN_DIR="$PLDL_RT/usr/lib/x86_64-linux-gnu/spa-0.2"
    export PIPEWIRE_MODULE_DIR="$PLDL_RT/usr/lib/x86_64-linux-gnu/pipewire-0.3"
