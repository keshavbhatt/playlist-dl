...
void FreedesktopNotifier::show(quint64 id, const core::Notification& n)
{
    ...
    if (n.sound) {
        hints.insert(u"sound-name"_s, u"complete-download"_s);
    }

    const QString appName =
        QGuiApplication::instance() != nullptr ? QGuiApplication::applicationDisplayName() : u"Red"_s;
    const QString appIcon = n.desktopEntry; // themed icon name = desktop id when installed

    QStringList actions{kDefaultAction, tr("Open")};
    for (const core::NotificationAction& action : n.actions) {
        actions << action.key << action.label;
    }
    QDBusMessage call = QDBusMessage::createMethodCall(kService, kPath, kInterface, u"Notify"_s);
    call << appName << quint32(0) << appIcon << n.title << n.body << actions << hints << n.timeoutMs;
...
void FreedesktopNotifier::onActionInvoked(uint dbusId, const QString& actionKey)
{
    ...
    if (actionKey == kDefaultAction || actionKey.isEmpty()) {
        Q_EMIT activated(it.value());
    } else {
        Q_EMIT actionInvoked(it.value(), actionKey);
    }
}
