    ...
    ui(new Ui::PlaylistSearch)
{
    ...
    ui->searchLineEdit->addAction(QIcon(":/icons/youtube-line.png"),QLineEdit::LeadingPosition);
    ui->searchLineEdit->setPlaceholderText(tr("Search YouTube playlist or paste Playlist Url"));
...
void PlaylistSearch::doSearch()
{
    if(ui->searchButton->text().contains("playlist",Qt::CaseInsensitive))
    {
        ...
        if(extractedId.trimmed().isEmpty()){
            QMessageBox::information(this,QApplication::applicationName()+" | Error","Unable to extract the Playlist ID for processing,\n"
...
void PlaylistSearch::processResult()
{
    ...
    if(reply->error() == QNetworkReply::NoError){
        ...
        if(jsonResponse.array().isEmpty()){
            //QMessageBox::critical(this,QApplication::applicationName()+" | Error","API:Empty response returned from API call. Please report to developer.");
            this->noResults();
        ...
        if(reply->error() == QNetworkReply::OperationCanceledError)
            return;
        QMessageBox::critical(this,QApplication::applicationName()+" | Error","An error occured while search "+
...
void PlaylistSearch::noResults()
{
    ...
    listTitle->setText(QString("<b>No result found for your query %1</b>").arg(ui->searchLineEdit->text()));
...
void PlaylistSearch::parseResult(const QJsonDocument jsonResponse)
{
    ...
    foreach (const QJsonValue &val, jsonArray)
    {
       ...
       PlayListItem *playlistItem = new PlayListItem(ui->resultsListWidget,n_manager);
       connect(playlistItem,&PlayListItem::selectItem,[=](QPoint itemPos)
       {
          ui->resultsListWidget->setCurrentItem(ui->resultsListWidget->itemAt(itemPos));
       });

       connect(playlistItem,&PlayListItem::viewPlaylist,[=](QString playlistId)
       {
          emit loadPlaylist(playlistId);
       });

       playlistItem->setObjectName("item_"+id);
       playlistItem->init(id,title,playlistThumbnail,author,authorId,videoCount,videoMeta);
...
{
    QString id;
    //QRegExp reg("(?:http|https:\\/\\/|)www\\.youtube\\.com\\/playlist\\?list=([a-zA-Z0-9_-]{1,})");
...
void PlaylistSearch::on_resultsListWidget_itemDoubleClicked(QListWidgetItem *item)
{
