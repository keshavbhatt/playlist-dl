...
PlaylistView::PlaylistView(QWidget *parent, QVariant playlist_id)
    : QWidget(parent), ui(new Ui::PlaylistView) {
  ui->setupUi(this);
...
void PlaylistView::flat_playlist(QVariant playlist_id) {
  ...
  connect(flatter, SIGNAL(readyReadStandardError()), this,
          SLOT(flattererror()));

  flatter->setArguments(QStringList()
                        << Engine::enginePath() << "--dump-single-json"
                        << "--flat-playlist" << url_str);
  _loader->start();
  ...
  if (flatter != nullptr) {
    ...
    QMessageBox::critical(
        ...
        "An error occured while processing playlist.\n\nError code " +
            QString::number(flatter->exitCode()) + "\n\n" + data);
  }
}
...
void PlaylistView::loadToView(const bool fromCache, const QString data) {
  ...
  ui->forceReload->setEnabled(fromCache);

  this->setWindowTitle(QApplication::applicationName() + " | " +
                       tr("Playlist - ") + playlist_name);
  ...
  foreach (const QJsonValue &value, jsonArray) {
    ...
    if (title.contains("[Deleted video]", Qt::CaseInsensitive) ||
      ...
      VideoItem *videoItem =
          new VideoItem(ui->resultsListWidget, this->networkManager_);
      videoItem->setObjectName("video_" + videoId);
      connect(videoItem, &VideoItem::itemCheckedChanged, this,
...
PlaylistView::~PlaylistView() { delete ui; }
...
void PlaylistView::loadPlaylist(const QString playlistId) {
  ...
  this->playlistCacheFileName =
      utils::returnPath("playlist_cache") + this->playlist_id;
...
void PlaylistView::updateStatusLabel() {
  ...
  ui->status_label->setText(QString::number(checkedCount) +
                            QString(checkedCount == 1 ? tr(" item selected")
                                                      : tr(" items selected")));
...
void PlaylistView::filterList(const QString &arg1, QListWidget *listWidget) {
  ...
  for (int i = 0; i < filteredItemsMetaList.count(); i++) {
    if (QString(filteredItemsMetaList.at(i))
...
void PlaylistView::fillFilteredItemsMetaList(QListWidget *listWidget) {
  for (int i = 0; i < listWidget->count(); i++) {
    QListWidgetItem *item = listWidget->item(i);
    ElidedLabel *title =
        listWidget->itemWidget(item)->findChild<ElidedLabel *>("title");
...
void PlaylistView::on_resultsListWidget_itemDoubleClicked(
    QListWidgetItem *item) {
...
void PlaylistView::on_downloadPushButton_clicked() {
  //--yes-playlist --playlist-items "3" -f "160+249" --merge-output-format "mp4"
  //-o "/tmp/test3/%(playlist_index)s-%(title)s-%(id)s.%(ext)s" --exec "mv {}
  /// tmp/test/" PLq3UZa7STrbpX13PljcNH6hmyrSbcMYK8

  emit downloadSelected(this->playlist_id,
                        getSelectedItemId(ui->resultsListWidget));
}
...
void PlaylistView::on_copyId_clicked() {
  ...
  clipboard->setText("https://www.youtube.com/playlist?list=" +
                     this->playlist_id);
}

void PlaylistView::on_forceReload_clicked() {

  if (!playlist_id.trimmed().isEmpty()) {
    this->playlistCacheFileName =
        utils::returnPath("playlist_cache") + this->playlist_id;

    QFile::remove(playlistCacheFileName);
    loadPlaylist(this->playlist_id);
  } else {
    qWarning() << "Unable to force reload playlist with id" << playlist_id;
  }
}
