...
void RemotePixmapLabel2::pixmapReceived()
{
             ...
             reply->error() == QNetworkReply::ContentAccessDenied )
    {
        ...
        qDebug() << "pixmap receiving error" << reply->errorString();
        setFallbackPixmap();
...
void RemotePixmapLabel2::setFallbackPixmap()
{
    if(this->fallbackUrlStr.isEmpty()==false)
        setPixmap(QPixmap(fallbackUrlStr).scaled(this->size(),
