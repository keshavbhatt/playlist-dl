...
//static on demand path maker
QString utils::returnExactPath(QString pathname)
{
...
{
        QObject *parentMainWindow = self->parent();
        while (!parentMainWindow->inherits("QMainWindow")){
            parentMainWindow = parentMainWindow -> parent();
        }
        return parentMainWindow;
}

//return true if given string is number(int float or double)
bool utils::is_number( std::string token )
{
