...
using pldl::core::LogSink;

Q_LOGGING_CATEGORY(lcTest, "pldl.test")
...
private Q_SLOTS:
    ...
    void ringBufferKeepsFormattedLines()
    {
        ...
        QVERIFY(last.contains(u"[pldl.test]"_s));
    ...
    void writesToFileAndRotates()
    {
        QTemporaryDir dir;
        QVERIFY(dir.isValid());
        const QString path = dir.filePath(u"sub/pldl.log"_s);
    ...
    {
        LogSink::setLogFile(u"/proc/definitely/not/writable/pldl.log"_s);
        QVERIFY(LogSink::logFilePath().isEmpty());
    ...
    void ringIsCapped()
    {
        // info, not debug: the test environment disables pldl.*.debug.
